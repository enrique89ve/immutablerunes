<script lang="ts">
	import type { StoredImage } from '@/protocol';
	...
	let { images, emptyMessage }: Props = $props();
	const defaultEmptyMessage = $derived(emptyMessage ?? t('profile.empty'));

	let selectedImage: ExploreImage | null = $state(null);
	let viewMode: 'grid' | 'large' = $state('large');

	function openImage(image: StoredImage) {
		selectedImage = {
			...image,
			blockNumber: image.blockNumber ?? 0,
			timestamp: new Date(image.createdAt).toISOString()
		};
	}
	...
	function formatRelativeTime(createdAt: number): string {
		...
		return new Date(createdAt).toLocaleDateString(locale, { month: 'short', day: 'numeric' });
	}
</script>
...
{#if images.length > 0}
	...
	{:else}
		...
		<div class="flex flex-wrap justify-center gap-4 px-2">
			{#each images as image (image.imageId)}
				<div class="relative group w-[calc(50%-0.5rem)] sm:w-[calc(33.333%-0.75rem)] md:w-[calc(25%-0.75rem)] lg:w-[calc(20%-0.8rem)] max-w-[180px]">
...
{:else}
	<div class="flex min-h-[400px] flex-col items-center justify-center border border-dashed border-text/20 rounded-lg bg-subtle/30">
		...
		<p class="font-mono text-sm text-text/60 mb-6 text-center max-w-xs">{defaultEmptyMessage}</p>
